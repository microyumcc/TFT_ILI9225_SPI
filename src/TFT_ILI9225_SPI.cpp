...
TFT_ILI9225_SPI::TFT_ILI9225_SPI(int8_t rst, int8_t rs, int8_t cs, int8_t sdi, int8_t clk, int8_t led) {
    _rst  = rst;
    _rs   = rs;
    _cs   = cs;
    _sdi  = sdi;
    ...
    _led  = led;
    #if defined(ESP8266) || defined(ESP32)
        _brightness = 1023; // Set to maximum brightness
    #else
...
TFT_ILI9225_SPI::TFT_ILI9225_SPI(int8_t rst, int8_t rs, int8_t cs, int8_t sdi, int8_t clk, int8_t led, uint16_t brightness) {
    _rst  = rst;
    _rs   = rs;
    _cs   = cs;
    _sdi  = sdi;
...
TFT_ILI9225_SPI::TFT_ILI9225_SPI(int8_t rst, int8_t rs, int8_t cs, int8_t led) {
    _rst  = rst;
    _rs   = rs;
    _cs   = cs;
    _sdi  = _clk = -1;
...
TFT_ILI9225_SPI::TFT_ILI9225_SPI(int8_t rst, int8_t rs, int8_t cs, int8_t led, uint16_t brightness) {
    _rst  = rst;
    _rs   = rs;
    _cs   = cs;
    _sdi  = _clk = -1;
...
#ifdef ESP32
void TFT_ILI9225_SPI::begin(SPIClass &spi)
#else
...
void TFT_ILI9225_SPI::_spiWrite(uint8_t b) {
    if(_clk < 0){
        HSPI_WRITE(b);
        return;
    }
...
void TFT_ILI9225_SPI::invert(boolean flag) {
    startWrite();
...
void TFT_ILI9225_SPI::setBacklight(boolean flag) {
    blState = flag;
#ifndef ESP32
...
void TFT_ILI9225_SPI::setDisplay(boolean flag) {
    if (flag) {
        startWrite();
...
uint16_t TFT_ILI9225_SPI::drawText(uint16_t x, uint16_t y, STRING s, uint16_t color, uint16_t bg_color) {
    ...
    for (uint8_t k = 0; k < s.length(); k++) {
        currx += drawChar(currx, curry, s.charAt(k), color, bg_color) + 1;
        if (currx + cfont.width >= _maxX) {
            currx = 0;
            curry += cfont.height;
        }
    }
...
void TFT_ILI9225_SPI::drawRGBBitmap(int16_t x, int16_t y,
  const uint16_t bitmap[], int16_t w, int16_t h) {
    startWrite();
...
void TFT_ILI9225_SPI::startWrite(void){
    SPI_BEGIN_TRANSACTION();
    SPI_CS_LOW();
}


void TFT_ILI9225_SPI::endWrite(void){
    SPI_CS_HIGH();
    SPI_END_TRANSACTION();
}
